...
async function test() {
  ...
  console.log('Merged Config in Registry:', JSON.stringify(app.context.plugins[pluginName].config, null, 2));

  const config = app.context.plugins[pluginName].config;

  // Verification
  if (config.hasOffer === false && 
      config.propertyStatus[0] === "for sale" && 
      config.newProjectField === 'hello' &&
      config.params.initialParam === 'default') {
    console.log('SUCCESS: Global Plugin Registry initialized with correctly merged external config!');
  } else {
